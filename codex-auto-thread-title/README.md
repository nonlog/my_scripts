...
On each `UserPromptSubmit` event the hook launcher exits quickly after starting a detached worker, before the main agent turn needs to finish. The worker then checks whether the thread still needs a title. For an eligible thread it:
...
- removes this hook's legacy `Stop` registration if present, then merges one short synchronous `UserPromptSubmit` launcher into `~/.codex/hooks.json`; the launcher immediately detaches the real worker;
...
Conceptually the installer adds this group to `hooks.UserPromptSubmit` (the real command contains absolute paths):
